...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    setQuestion(state, items) {
      var useritem = items.filter(item => item.coursecode == localStorage.getItem("coursecode"))
    ...
    setStudent(state, items) {
      var useritem = items.filter(item => item.id == state.studentID)
      state.studentDetails = useritem;
    },
  ...
  actions: {
    addCourse({ commit }, payload) {
      ...
      db.collection('courselist').add(payload).then((response) => {
        if (response) {
    ...
    deleteCourse({ commit }, id) {
      if (id) {
        ...
        db.collection("courselist").doc(id).delete().then(function () {
    ...
    setCourses(context) {
          let items = []
          db.collection('courselist').orderBy('created_at').onSnapshot((snapshot) => {
            items = []
            snapshot.forEach((doc) => {
              items.push({ id: doc.id, coursecode: doc.data().coursecode, coursename: doc.data().coursename, coursesem: doc.data().coursesem })
            })
    ...
    addTeacher({ commit }, payload) {
      ...
      db.collection('teacherlist').add(payload).then((response) => {
        if (response) {
          // eslint-disable-next-line no-console
          console.log(response);
        }
    ...
    deleteTeacher({ commit }, id) {
      if (id) {
        ...
        db.collection("teacherlist").doc(id).delete().then(function () {
    ...
    setTeacher(context) {
          let items = []
          db.collection('teacherlist').orderBy('created_at').onSnapshot((snapshot) => {
            items = []
            snapshot.forEach((doc) => {
              items.push({ id: doc.id, teachersem: doc.data().teachersem, teachersect: doc.data().teachersect, teachername: doc.data().teachername })
            })
    ...
    addQuestion({ commit }, payload) {
      ...
      db.collection('questionlist').add(payload).then((response) => {
        if (response) {
          // eslint-disable-next-line no-console
          console.log(response);
        }
    ...
    deleteQuestion({ commit }, id) {
      if (id) {
        ...
        db.collection("questionlist").doc(id).delete().then(function () {
    ...
    setQuestionlist(context) {
          let items = []
          db.collection('questionlist').orderBy('created_at').onSnapshot((snapshot) => {
            items = []
            snapshot.forEach((doc) => {
              items.push({ id: doc.id, question: doc.data().question, coursecode: doc.data().coursecode })
            })
    ...
    addStudent({ commit }, payload) {
      ...
      db.collection('studentlist').add(payload).then((response) => {
        if (response) {
